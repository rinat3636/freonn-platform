...
## Map / Leaflet testing
- OSM tiles render correctly in-browser once the app's CSP allows the tile host. Earlier "gray tiles" were caused by the app's own Content-Security-Policy `img-src` in `server/_core/securityHeaders.ts` NOT listing the tile host — NOT a VM egress block (that earlier diagnosis was wrong). The fix added `https://*.tile.openstreetmap.org https://tile.openstreetmap.org https://*.basemaps.cartocdn.com` plus `blob:` to `img-src`.
- Confirmed working (commit d917c43): on `/map`, 35/35 `.leaflet-tile` images loaded with `naturalWidth=256`, marker rendered, and **zero CSP violations** in the console. Streets/labels visibly render.
- If tiles ever go gray again, diagnose via the browser console FIRST: look for a CSP violation naming `tile.openstreetmap.org` (→ it's the app CSP `img-src` again) before suspecting network/egress. Also verify via console: `.leaflet-container` non-zero size (proves `invalidateSize`), `.leaflet-tile` count > 0, and marker count matches projects with coordinates (proves `fitBounds`).
- The floating object panel on `/map` lists all projects; those without resolved coordinates are disabled and show «нет координат».
